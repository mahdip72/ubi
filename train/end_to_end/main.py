...
def train(dataloader, model, tools, e):
    ...
    for batch, (x, y, w) in enumerate(dataloader):
        ...
        if tools['mixed_precision']:
            if tools['sam']:
                compute_sam(tools, loss, model, x, y, w)
            else:
...
def valid(dataloader, model, tools, e):
    ...
    with torch.no_grad():
        for x, y, w in dataloader:
            x, y = x.to(tools['device']), y.to(tools['device'])
            pred = model(x)
...
def main(window_size, batch, seed):
    configs = {
        ...
        'save_path': './results/lstm_final_3',
    ...
    train_df = pd.read_csv(train_path, usecols=['window', 'label'])
    valid_df = pd.read_csv(valid_path, usecols=['window', 'label'])
    test_df = pd.read_csv(test_path, usecols=['window', 'label'])
    ...
    training_data = CustomDataset(train_df, tokenizer, configs['pretrained_weights'])
    validation_data = CustomDataset(valid_df, tokenizer, configs['pretrained_weights'])
    test_data = CustomDataset(test_df, tokenizer, configs['pretrained_weights'])
    ...
    del training_data, train_dataloader, validation_data, valid_dataloader, test_data, test_dataloader
    del train_df, valid_df, test_df,  tools
